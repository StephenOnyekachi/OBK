...
def View_user(request, pk):
    ...
    if request.method == 'POST':
        ...
        user.save()
        messages.success(request, f'{user.username} balance is now {user.balance}')
        return redirect('all_users')
    context = {
        'user':user,
    }
...
def Transfer_finish(request):
    ...
    if request.method == 'POST':
        ...
        infor = Transfer_otp.objects.filter(user=user)

        if Transfer_otp.objects.filter(user=user, otp_code=otp).exists():

            print(Transfer_otp.receiver)
            credits = Transfer_otp.objects.all().get()
...
def Confirm(request):
    ...
    if request.method == 'POST':
        ...
        if Transfer_otp.objects.filter(user=user, otp_code=otp).exists():
            ...
            print('falied')
            messages.error(request, 'incorrect otp code redo it with the otp code send before')
            return redirect('transfer')
